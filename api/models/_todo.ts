 ...
 *　全件取得
 */
export async function getAll(): Promise<_todo[]> {
 ...
 * @param id
 */
export async function get({id}: Pick<_todo, "id">): Promise<Result<_todo>>{
 ...
 * @param title
 */
export async function create({title}: Pick<_todo, "title">): Promise<true> {
  const todos = await getAll();
 ...
 * @param todos
 */
async function updateAll(todos: _todo[]): Promise<true> {
 ...
 * @param params
 */
export async function update(params: Partial<_todo> & Pick<_todo, 'id'>): Promise<Result<true>>  {
  const todos = await getAll();
 ...
 * @param id
 */
export async function remove({id}: Pick<_todo, 'id'>): Promise<Result<true>>  {
  const todos = await getAll();
  const todosMap = toMap(todos);
